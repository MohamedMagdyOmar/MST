...
def get_all_diacritics_from_db():
    ...
    db_distinct_diacritics = cur.fetchall()
    db_distinct_diacritics = [eachTuple[0] for eachTuple in db_distinct_diacritics]

    return db_distinct_diacritics


def get_actual_diacritics(list_of_distinct_diacritics, neurons_locations_with_highest_output):
...
def get_db_un_diacritized_chars(__sentence_number):
    list_of_un_diacritized_letters_query = "select UnDiacritizedCharacter from parseddocument where " \
                                                 "LetterType='testing' and SentenceNumber = " + str(__sentence_number)

    cur.execute(list_of_un_diacritized_letters_query)

    db_un_diacritized_letters_before_sukun_correction = cur.fetchall()
    db_un_diacritized_letters_before_sukun_correction = \
        [eachTuple[0] for eachTuple in db_un_diacritized_letters_before_sukun_correction]

    return db_un_diacritized_letters_before_sukun_correction


def get_actual_letters(un_diacritized_chars, actual_diacritics_before_sukun_correction):
    list_of_actual_letters = []
    if len(un_diacritized_chars) != len(actual_diacritics_before_sukun_correction):
        raise 'Bug In get_actual_letters'

    for each_char, each_diacritics in zip(un_diacritized_chars, actual_diacritics_before_sukun_correction):
        list_of_actual_letters.append((each_char + each_diacritics))

    return list_of_actual_letters
...
if __name__ == "__main__":
    ...
    for file_name in result:
        ...
        NeuronsLocationsWithHighestOutput = \
            get_neurons_numbers_with_highest_output_value(rnn_output_for_one_seq)
        connect_to_db()
        ListOfDistinctDiacritics = get_all_diacritics_from_db()
        ActualDiacriticsBeforeSukunCorrection = get_actual_diacritics(ListOfDistinctDiacritics,
                                                                      NeuronsLocationsWithHighestOutput)
        UnDiacritizedChars = get_db_un_diacritized_chars(sentence_number)
        ActualLettersBeforeSukunCorrection = get_actual_letters(UnDiacritizedChars,
                                                                    ActualDiacriticsBeforeSukunCorrection)
        ExpectedLettersBeforeSukunCorrection = get_expected_letters(sentence_number)

        actual_letters_after_sukun_correction, expected_letters_after_sukun_correction = \
            sukun_correction(ActualLettersBeforeSukunCorrection,
                             ExpectedLettersBeforeSukunCorrection)
        ...
        # dictionary correction
        list_of_words_in_sent_after_sukun_correction = reform_word_after_sukun_correction(location_of_each_char_for_actual_op)
        ...
        list_of_error_without_counting_last_letter = get_diacritization_error_without_counting_last_letter(
            list_of_chars_in_sent_after_sukun_and_dict_correction,
            location_of_each_char_for_expected_op,
            selected_sentence)
